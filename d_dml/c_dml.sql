...
# cf) ddl : create(생성), alter(수정), drop(삭제), truncate(데이터삭제)
...
/*
	1. 데이터 삽입 (INSERT)
    : 테이블 행 데이터(레코드)를 입력하는 키워드alter
    # 기본 형식 #
    insert into 테이블 명 (열1, 열2, ...)
    values (값1, 값2 ...);
    
    -- 열 이름을 생략 할 경우 values 키워드 뒤 값들의 순서는
...
# 1. Null 허용 - 자동 null 값 지정
# 2. NOT Null 지정 - 오류 

# cf) auto_increment
# : 열을 정의할 때 1부터 1씩 증가하는 값을 입력 
# >> insert에서는 해당 열이 없다고 가정하고 입력!
# >> 해당 칼럼은 반드시 PK키로 지정되어야 함 

create table `example02` (
	-- 칼럼명 데이터타입 PRIMARY KEY auto_increament
...
/*
    ...
    cf) where 조건이 없는 경우 해당 열(컬럼)의 데이터는 해당 값으로 모두 변경 !examlple01
*/
...
-- safe update mode를 설정하는 코드
 ...
 # cf) 여러 칼럼의 값을 한번에 설정하는 경우 콤마(,)로 분리하여 나열 
